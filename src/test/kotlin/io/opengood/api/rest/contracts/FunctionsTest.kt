...
class FunctionsTest : FunSpec({
    ...
    test("actionFailedResponse function returns response entity with bad request status, failed state, message, and empty list of data") {
    ...
    test("dataByIdFailedResponse function returns response entity with bad request status, failed state, message, and empty map of data") {
    ...
    test("dataFailedResponse function returns response entity with bad request status, failed state, message, empty pages, empty records, and empty list of data") {
        val response = dataFailedResponse(message = "Error")

        with(response) {
            statusCode shouldBe HttpStatus.BAD_REQUEST
            body shouldBe DataResponse(
                state = OperationState.FAILED,
                message = "Error",
    ...
    test("dataSuccessResponse function returns response entity with ok status, success state, no message, empty pages, empty records, and list of data") {
        val data = listOf(
            mapOf(
                "product_id" to 1,
                "name" to "Product 1",
